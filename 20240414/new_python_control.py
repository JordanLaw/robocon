...
# PID_value_fl = [0.1, 0.0, 0.0]
PID_value_fl = [0.2, 0.0, 0.0]
# PID_value_fl = [0.265, 0.01, 0.0]

# prev_time, prev_e, integral_accumulate, four bytes of speed
PID_fr = [0, 0, 0, 0, 0, 0, 0]
# PID_value_fr = [0.1, 0.0, 0.0]
PID_value_fr = [0.2, 0.0, 0.0]
# PID_value_fr = [0.265, 0.01, 0.0]

# prev_time, prev_e, integral_accumulate, four bytes of speed
PID_br = [0, 0, 0, 0, 0, 0, 0]
# PID_value_br = [0.1, 0.0, 0.0]
PID_value_br = [0.2, 0.0, 0.0]
# PID_value_br = [0.265, 0.01, 0.0]

# prev_time, prev_e, integral_accumulate, four bytes of speed
PID_bl = [0, 0, 0, 0, 0, 0, 0]
# PID_value_bl = [0.1, 0.0, 0.0]
PID_value_bl = [0.2, 0.0, 0.0]
...
while True:
    ...
    if step == 0:
        ...
        print(f"WS1: {wheel_command[0]:.2f} WS2: {wheel_command[1]:.2f} WS3: {wheel_command[2]:.2f} WS4: {wheel_command[3]:.2f}")
        print(f"WA1: {wheel_command[4]:.2f} WA2: {wheel_command[5]:.2f} WA3: {wheel_command[6]:.2f} WA4: {wheel_command[7]:.2f}")
    ...
    if tick_per_round < actual_data[0] < -tick_per_round or tick_per_round < actual_data[1] < -tick_per_round or tick_per_round < actual_data[2] < -tick_per_round or tick_per_round < actual_data[3] < -tick_per_round:
        emer_stop()
