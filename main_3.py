...
# Stop the robot after ESP32 connected
...
def move_command(line_cx, line_cy):
    # Control robot follow the white line

    # move  forward
    if 50 <= line_cx <= 70:
        value_line = [0xFF, 0x00, 0x40, 0x00, 0x00, 0x00]
        ser.write(serial.to_bytes(value_line))
        print("F")

    # move left
    elif line_cx <= 50 and line_cy <= 30:
        value_line = [0xFF, 0x40, 0x00, 0x00, 0x00, 0x00]
        ser.write(serial.to_bytes(value_line))
        print("L")

    # move right
    elif line_cx >= 70 and line_cy <= 30:
        value_line = [0xFF, 0xC9, 0x00, 0x00, 0x00, 0x00]
        ser.write(serial.to_bytes(value_line))
        print("R")

    # rotate left
    elif line_cx <= 50:
        value_line = [0xFF, 0x00, 0x00, 0xC0, 0x00, 0x00]
        ser.write(serial.to_bytes(value_line))
        print("RL")

    # rotate right
    elif line_cx >= 70:
        value_line = [0xFF, 0x00, 0x00, 0x30, 0x00, 0x00]
        ser.write(serial.to_bytes(value_line))
        print("RR")


def move_basket(white_area, basket,pass_numberLine, Wcontours, extTop, state, counter):
    # Control the robot move to the place in front of the chosen basket
    # Basket2 is the middle of the basket

    # move right
    if basket > 2:
        
        # Count how many times passed the white line
        if white_area >= 100:
            counter = 1
        ...
        value_mov = [0xFF, 0xC9, 0x00, 0x00, 0x00, 0x00]
        ser.write(serial.to_bytes(value_mov))
        print("R1")
    ...
    if basket < 2:
        ...
        value_mov = [0xFF, 0x40, 0x00, 0x00, 0x00, 0x00]
        ser.write(serial.to_bytes(value_mov))
        print("R1")
...
while True:
    ...
    while detect_time <= 25:
        ...
        # Put the ball value in the correct basket array
        for z in range(len(ballstatus)):
            if len(basket) >= 1:
                if basket[0]['x1'] <= ballstatus[z]['cx'] <= basket[0]['x2']:
                    basket1ball.append(ballstatus[z])
            if len(basket) >= 2:
                if basket[1]['x1'] <= ballstatus[z]['cx'] <= basket[1]['x2']:
                    basket2ball.append(ballstatus[z])
            if len(basket) >= 3:
                if basket[2]['x1'] <= ballstatus[z]['cx'] <= basket[2]['x2']:
                    basket3ball.append(ballstatus[z])
            if len(basket) >= 4:
                if basket[3]['x1'] <= ballstatus[z]['cx'] <= basket[3]['x2']:
                    basket4ball.append(ballstatus[z])
            if len(basket) >= 5:
                if basket[4]['x1'] <= ballstatus[z]['cx'] <= basket[4]['x2']:
        ...
        # Add all the ball value inside an array for checking
        ...
        # check all the basket situation
        for x in range(5):
        ...
        # Provide the basket status to AI program and decide which basket is chosen
        if len(basket) > 0:

            # Obtain the observation data which AI model need
            obs.extend(basket_status)
            obs.extend(basket_full)
            print(obs)
            obs = obs + list(prev_actions)

            # Use AI model to decide path
            if len(obs) >= 40: # Error exist if obs is not equal to 40
    ...
    # Find the highest times the basket AI chose
    ...
    # Control the robot base on the action
    while action >= 0 and action <= 4:
        ...
        if len(Wcontours) > 0:  # detected white line
            ...
            # Find the conner of the white contour
        ...
        # Move and follow the white line at the beginning point until meet the cross
        if state == 0:
            move_command(cx, cy)
        ...
        # Adjust the robot position face to the cross
        if state == 1:
            move_command(cx, cy)
        ...
        # Move the robot to the place in front of the chosen basket
        if state == 2:
            if basket_target == 2:
                state += 1
            else:
        ...
        # Stop the robot in front of the basket
        if state == 3:
